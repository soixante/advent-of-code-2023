...
$datas = "Game 1: 1 green, 2 blue; 13 red, 2 blue, 3 green; 4 green, 14 red
...
function processSample(string $sample)
{
    $matching = [
        'maxr' => 12,
        'maxg' => 13,
    ...
    foreach ($samples as $oneSample) {
        $key = getKeyFromSample($oneSample);
        $maxBySamples = getMaxBySample($oneSample);

        if ($key > 0) {
            $power = ($maxBySamples['maxr'] ?:1) * ($maxBySamples['maxg']?:1) * ($maxBySamples['maxb']?:1);
            printf("\nGame %d: %d (%d / %d / %d)", $key, $power, $maxBySamples['maxr'], $maxBySamples['maxg'], $maxBySamples['maxb']);
            $powerSum += $power;
        }

        $sampleOk = true;
        if ($maxBySamples['maxr'] && $maxBySamples['maxr'] > $matching['maxr']) {
            $sampleOk = false;
        }
        if ($maxBySamples['maxg'] && $maxBySamples['maxg'] > $matching['maxg']) {
            $sampleOk = false;
        }
        if ($maxBySamples['maxb'] && $maxBySamples['maxb'] > $matching['maxb']) {
            $sampleOk = false;
        }

        if ($sampleOk) {
            $matchingSampleIds[] = $key;
        }
    }


    $sumIds = 0;
    foreach ( $matchingSampleIds as $id)  {
        $sumIds += $id;
    }
...
function getKeyFromSample(string $oneSample): int
{
    $key = 0;
    if (preg_match('/^Game (?<gameId>\d+)/', $oneSample, $matches)) {
        $key = $matches['gameId'];
    }
...
function getMaxBySample(string $oneSample): array
{
    $tries = explode(";", $oneSample);
    $maxr = 0;
    $maxg = 0;
    $maxb = 0;
    foreach ($tries as $try) {
        if (preg_match('/(?<r>\d+)\s*r/', $try, $matches)) {
            $maxr = $matches['r'] > $maxr ? $matches['r'] : $maxr;
        }
        if (preg_match('/(?<g>\d+)\s*g/', $try, $matches)) {
            $maxg = $matches['g'] > $maxg ? $matches['g'] : $maxg;
        }
        if (preg_match('/(?<b>\d+)\s*b/', $try, $matches)) {
            $maxb = $matches['b'] > $maxb ? $matches['b'] : $maxb;
        }
    }
